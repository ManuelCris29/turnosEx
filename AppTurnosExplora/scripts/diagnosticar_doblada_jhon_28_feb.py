"""
Script de diagnóstico para verificar la doblada de jhon.areiza el 28 de febrero de 2026
"""
import os
import sys
import django
...
def diagnosticar_doblada_jhon_28_feb():
    ...
    if turnos.exists():
        ...
        for turno in turnos:
            ...
            if turno.jornada:
                print(f"      Horario jornada: {turno.jornada.hora_inicio} - {turno.jornada.hora_fin}")
    ...
    es_doblada_turnos = 'AM' in jornadas and 'PM' in jornadas if tiene_turnos else False
    
    if tiene_turnos and es_doblada_turnos and (tiene_doblada_solicitante or tiene_doblada_receptor):
        print("   ✅ CORRECTO: Tiene turnos AM+PM Y tiene doblada aprobada")
        print("      → El sistema DEBE mostrar 'Doblada Existente Detectada'")
        print("      → Debe mostrar opciones de cesión parcial/total")
    elif tiene_turnos and es_doblada_turnos and not tiene_doblada_solicitante and not tiene_doblada_receptor:
        print("   ⚠️ PROBLEMA: Tiene turnos AM+PM pero NO tiene doblada aprobada")
        print("      → Es su jornada normal (no doblada aprobada)")
        print("      → El sistema NO debe mostrar 'Doblada Existente Detectada'")
        print("      → PERO el usuario quiere poder ceder parte de su jornada")
        print("      → SOLUCIÓN: Permitir cesión parcial incluso sin doblada aprobada")
    elif tiene_turnos and not es_doblada_turnos:
        print("   ✅ CORRECTO: Tiene turnos pero NO es doblada (solo una jornada)")
        print("      → Es su jornada normal")
...
if __name__ == '__main__':
    diagnosticar_doblada_jhon_28_feb()
