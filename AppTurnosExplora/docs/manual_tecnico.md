...
| `DeudaPermisoMes` | `permiso`, `explorador` (desnormalizado a propósito, `:256-258`), `anio`, `mes`, `minutos_generados`, `minutos_pagados`, `ocurrencias`, `estado` (los mismos cinco valores que `DeudaCorporativa`, `:242-251`), `fecha_pago`, `sancion_consumidora` (`:274`), `fecha_consumo` (`:277`), `historial` | `ordering = ['anio', 'mes', 'permiso']` (`permisos/models.py:283`) | Un permiso × un mes: es la unidad que permite reclamar mes a mes y admitir **pago parcial** ([ADR 011](./03-arquitectura/adr/011-sancion-independiente-del-pago.md)). `fecha_pago` es la del pago que la **saldó**, no la del primer abono (`:270-272`). `PermisoEspecial.pagado` es un **agregado derivado** de estas filas: cualquier cosa que cambie sus estados tiene que rehacer el roll-up, o el permiso sigue figurando impagado en las pantallas que leen el agregado. `condonada` (`:249`) marca la deuda que perdonó el supervisor al levantar la sanción del mes |

### 5.3 Migraciones

`solicitudes` tiene 36 archivos en `solicitudes/migrations/`. El orden de dependencias entre
apps es obligatorio y no se puede invertir: `permisos/models.py:6` importa `SolicitudCambio` y
...
`DeudaPermisoMes` (`permisos/models.py:249`), y servicio
...
| Nota del PDH obligatoria | `empleados/forms.py:378,380-383` (`required = True` + `clean_comentario`), `empleados/views/pdh.py:84,133` | En la vista se valida **después** de fecha y explorador, a propósito, para no tapar esos mensajes |
...
| **Modelos.** Seis campos nuevos, **idénticos** en `SolicitudCambio` (`solicitudes/models.py:166-195`) y `PermisoEspecial` (`permisos/models.py:127-142`) | `cancelacion_estado`, `cancelacion_solicitada_por` / `_en`, `cancelacion_respondida_por` / `_en`, `cancelacion_motivo`. Migraciones `solicitudes/0034` y `permisos/0008`, ya aplicadas, aditivas y también sobre las tablas históricas. Después, `permisos/0009` añadió `PermisoEspecial.fecha_aprobacion` (misma forma: aditiva, histórica, depende de la `0008`) | Dos modelos con el mismo bloque de campos y sin abstracción común: cualquier cambio hay que hacerlo dos veces |
