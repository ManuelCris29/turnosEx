...
Guía completa: [MANUAL_DOCKER_LOCAL.md](./05-referencia/deployment/03-operacion/MANUAL_DOCKER_LOCAL.md).
...
|---|---|---|---|
| HTTP | `*/views/` | Parsear la petición, comprobar sesión y permisos, devolver JSON o HTML. En el alta de solicitudes la traducción a JSON está en un único punto, `solicitudes/views/resultado_http.py:12` | No contiene reglas de negocio |
...
| Validadores | `solicitudes/services/validators/` | Reglas reutilizables entre tipos. Dos familias: **primitivas** que lanzan `ValidationError` (`doblada_validator.py`) y **tramos de flujo** que devuelven el mensaje (`doblada_flujo_validator.py`) | No escribe |
| Servicios de aplicación | `solicitudes/services/*_aplicacion_service.py` | Materializa y revierte turnos y deudas | No valida |
| Consultas | Managers y QuerySets del propio modelo (`solicitudes/models.py:136,149,256`) | Encapsular los `select_related`/filtros reutilizados | No decide |
...
(`config/settings.py:89-90`), por delante incluso del identificador.
...
(`solicitudes/services/solicitud_factory.py:131`) busca en cascada: campo `codigo_estrategia`
normalizado (`:143`), `codigo_estrategia` directo (`:154`), nombre directo (`:176`) y, si nada casa,
estrategia por defecto con `warning` en el log (`solicitudes/services/solicitud_factory.py:190-195`).
...
**Consultas encapsuladas, sin capa de repositorios.** Hubo un paquete
`solicitudes/repositories/` (y otro en `empleados/`) que envolvía el ORM. Se **retiró**: de sus 14
métodos solo 2 tenían llamador, y el resto era indirección que había que leer dos veces para
descubrir un `filter`. Lo que se reutiliza de verdad vive donde Django lo pone, en el manager del
modelo: `SolicitudCambioQuerySet.con_relaciones()` (`solicitudes/models.py:149`), instalado como
manager por defecto en `SolicitudCambio.objects` (`solicitudes/models.py:256`). El mismo camino que
...
repositorio, 43 patrones). Se resume en [§ 16](#16-zonas-frágiles-y-bugs-conocidos).
...
| Settings partidos por entorno | Un archivo único gobernado por `ENVIRONMENT` | `config/settings.py:23` |
...
├── PROTECTION_PATTERNS.md          43 patrones de concurrencia, idempotencia y seguridad
...
| `empleados` | Personas, jornadas, roles, salas, competencias, restricciones, sanciones, PDH y horas a favor (vistas) | `empleados/models.py` (303 líneas), `empleados/views/` | `core` |
...
erDiagram
    ...
    CreditoHoras }o--o{ PDH : creditos_consumidos
    Empleado ||--o{ CreditoHoras : "se le deben horas"
...
propia solicitud (`solicitudes/models.py:171,238-253`). A la izquierda queda el calendario:
...
| `TurnoArchivo` | Espejo de `Turno` + `fecha_archivado`, `turno_original_id`; `sala` también **nullable** | `turno_arch_exp_fecha_idx`, `turno_arch_fecha_idx` (`:227-230`) | Sin `CheckConstraint` a propósito: solo recibe copias ya validadas (`:218-219`) |
...
| `DeudaCorporativa` | `explorador`, `solicitud_origen` (`SET_NULL`), `minutos` (30), `fecha_doblada`, `estado` (`activa` / `pagada` / `consumida_por_sancion` / `condonada` / `cancelada`, `:706-720`), `fecha_pago`, `comentario`, `sancion_consumidora` (FK `SET_NULL` a `SancionEmpleado`, `:768`), `fecha_consumo` (`:771`) | `deuda_corp_exp_estado_idx`, `deuda_corp_fecha_estado_idx`, `deuda_corp_fecha_dob_idx` (`:695-699`) | Deuda **con la empresa**. `obtener_deuda_total()` suma las `activa`. **Cinco finales, no tres**, y cada uno dice algo distinto: `pagada` (hubo PDH), `consumida_por_sancion` (la extinguió el castigo **cumplido**), `condonada` (el supervisor **levantó** la auto-sanción de ese mes y con ella la deuda, `:719`; motivo de que sea estado propio en `:713-718`) y `cancelada` (corrección técnica: deuda huérfana, fin de semana deshecho). Los dos de extinción por sanción comparten los campos de trazabilidad —`sancion_consumidora` y `fecha_consumo`, la sanción responsable es una sola— y es el `estado` el que separa lo cumplido de lo perdonado (`:765-771`). Ver § 8.3 y el [ADR 013](./03-arquitectura/adr/013-condonacion-al-levantar-la-sancion.md) |
...
| Modelo | Campos clave | Constraints e índices | Notas de negocio |
|---|---|---|---|
| `PDH` | `explorador`, `solicitud` (opcional), `fecha`, `horas` (decimal 5,2), `supervisor`, `tipo_registro` (`pago_horas`), `comentario`, `deudas_pagadas` (M2M a `DeudaCorporativa`), `permisos_pagados` (M2M a `PermisoEspecial`), `creditos_consumidos` (M2M a `CreditoHoras` por `ConsumoCreditoHoras`, `:37-40`) | `pdh_exp_fecha_idx`, `pdh_solicitud_idx`, `pdh_fecha_idx` (`:47-50`) | `clean()` exige `0 < horas <= 24` (`:53-58`). Las horas del PDH son la suma de las deudas que salda (`:22-24`) |
| `PermisoEspecial` | `empleado`, `tipo` (5 valores), `es_permanente`, `fecha_inicio`, `fecha_fin`, `dias_semana` (CSV), `tiempo`, `especificacion`, `cubre` (`SET_NULL`), `motivo`, `estado`, `pagado`, `fecha_pago`, `supervisor`, `comentario_supervisor`, `jornada_trabaja`, `fecha_compensacion`, `snapshot_turnos_previos`, `fecha_aprobacion` (`DateTimeField`, `null=True blank=True`, `:94-97`) y el mismo bloque de **cancelación consensuada** que `SolicitudCambio` —`cancelacion_estado`, `cancelacion_solicitada_por`/`_en`, `cancelacion_respondida_por`/`_en`, `cancelacion_motivo`— (`:114-134`) | `perm_esp_emp_estado_idx`, `perm_esp_estado_idx`, `perm_esp_fecha_idx` (`:173-175`) | El tipo `MEDIA_JORNADA_TEMPORADA` parte el día completo de temporada en dos y **no genera deuda** (`tiempo=0`, `:98-107`). `horas_totales()` multiplica por las ocurrencias si es permanente (`:149-166`). `clean()` valida el rango (`:178-183`). `fecha_aprobacion` sella el instante en que el permiso pasó a `APROBADO` y es el equivalente de `SolicitudCambio.fecha_resolucion`: de él cuelga el plazo de 24 h para pedir la cancelación. No sirve `actualizado_en`, que es `auto_now` (`:137`) y lo reiniciaría cualquier guardado posterior (`permisos/models.py:96-104`). En la cancelación consensuada la contraparte **no es un receptor** —el permiso no tiene—, sino el **supervisor**: es quien lo aprobó y quien responde por la cobertura del día (`permisos/models.py:121-125`) |
| `DeudaPermisoMes` | `permiso`, `explorador` (desnormalizado a propósito, `:263-265`), `anio`, `mes`, `minutos_generados`, `minutos_pagados`, `ocurrencias`, `estado` (los mismos cinco valores que `DeudaCorporativa`, `:249-258`), `fecha_pago`, `sancion_consumidora` (`:281`), `fecha_consumo` (`:284`), `historial` | `ordering = ['anio', 'mes', 'permiso']` (`permisos/models.py:290`) | Un permiso × un mes: es la unidad que permite reclamar mes a mes y admitir **pago parcial** ([ADR 011](./03-arquitectura/adr/011-sancion-independiente-del-pago.md)). `fecha_pago` es la del pago que la **saldó**, no la del primer abono (`:277-279`). `PermisoEspecial.pagado` es un **agregado derivado** de estas filas: cualquier cosa que cambie sus estados tiene que rehacer el roll-up, o el permiso sigue figurando impagado en las pantallas que leen el agregado. `condonada` (`:256`) marca la deuda que perdonó el supervisor al levantar la sanción del mes |
| `CreditoHoras` | `explorador`, `fecha_hecho` (día en que trabajó de más), `minutos_otorgados`, `minutos_consumidos` (roll-up), `motivo`, `otorgado_por`, `estado` (`activo` / `consumido` / `anulado`, `:373-377`), `historial` | `ordering = ['fecha_hecho', 'id']` (`permisos/models.py:408`), índice `credito_exp_estado_idx` (`:410`) | **La dirección contraria de la deuda: lo que la corporación le debe AL explorador.** Es una **bolsa**, no un ajuste a una deuda concreta: sobrevive al pago y el remanente queda para deudas futuras. El `ordering` **no es cosmético** — es lo que implementa el consumo **FIFO**, así que cambiarlo cambia qué crédito se gasta primero. `minutos_pendientes` (`:414-416`) nunca es negativo. [ADR 014](./03-arquitectura/adr/014-horas-a-favor-del-explorador.md). Ver § 7.1 y § 8.2 |
| `ConsumoCreditoHoras` | `pdh` (`CASCADE`), `credito` (`PROTECT`), `minutos` | `UniqueConstraint(pdh, credito)` = `consumo_credito_horas_unico` (`permisos/models.py:453`) | Tabla intermedia con importe, **copia estructural de `PagoDeudaPermisoMes`** y por el mismo motivo: el consumo es parcial y puede repartirse entre varios créditos, así que sin el importe por fila borrar el PDH no sabría cuánto devolver a cada uno. `PROTECT` impide borrar un crédito que ya cubrió un pago |

### 5.3 Migraciones

`solicitudes` tiene 36 archivos en `solicitudes/migrations/`. El orden de dependencias entre
apps es obligatorio y no se puede invertir: `permisos/models.py:7` importa `SolicitudCambio` y
`solicitudes/models.py:224,227` apunta a `'turnos.Turno'` en dos claves foráneas
(por cadena, no por import), así que el orden es
...
- `turnos/migrations/0016_alter_turno_sala_alter_turnoarchivo_sala.py` pone `null=True, blank=True`
  en `Turno.sala` y `TurnoArchivo.sala` (`:17-23`). Es **relajar** una restricción, así que no
  puede fallar por datos existentes y no necesita limpieza previa. Motivo: la sala es
  **informativa** —dice en qué espacio tiene competencia el explorador, no condiciona el turno—
  y siendo `NOT NULL` hacía fallar la **aprobación de solicitudes** de cualquiera sin competencia
  cargada, porque `obtener_sala_explorador_fecha` lanzaba `ValidationError`
  (`turnos/models.py:56-63`). Cuando no hay sala, la interfaz muestra **"Por asignar"**
  (`turnos/services/doblada_turno_service.py:39,76`). Cubierto por `turnos/tests/test_sala_opcional.py`.
...
- `permisos/migrations/0011_alter_deudapermisomes_estado_and_more.py` y
  `solicitudes/migrations/0039_alter_deudacorporativa_estado_and_more.py` añaden el estado
  `'condonada'`, y `solicitudes/migrations/0040_alter_notificacion_tipo.py` añade el tipo de

- `permisos/migrations/0012_creditohoras_consumocreditohoras_and_more.py` crea `CreditoHoras`,
  `ConsumoCreditoHoras`, su histórico y el M2M `PDH.creditos_consumidos`.
  `permisos/migrations/0013_alter_creditohoras_minutos_otorgados_and_more.py` mueve los topes
  de 0 a 24 h desde `Model.clean()` a **validadores del campo**: es **no-op en SQL**
  (`sqlmigrate` lo confirma), solo cambia metadatos de validación. El porqué está en § 16.2.
  notificación `'sancion_levantada'`. **No son delicadas, y se listan aquí para que nadie las
  audite dos veces:** son solo `AlterField` sobre `estado`, su tabla histórica y
  `Notificacion.tipo`, y Django trata `choices` como un
  atributo que no vive en la base (`non_db_attrs`), así que no emiten `ALTER TABLE` ni pueden
  fallar sobre datos existentes. Sin backfill: ninguna fila anterior es `'condonada'`.
...
(`config/urls.py:24-38`). `__debug__/` solo existe con `DEBUG=True` (`config/urls.py:38-44`).
...
| GET | `/health/ready/` | `core.health.readiness` (`config/urls.py:27`) | ninguno | Estado verificando la conexión |
...
competencias, restricciones, sanciones, PDH, horas a favor) y de permisos especiales. La referencia de la
Las rutas del crédito son `/empleados/credito-horas/` (`credito_list`), `.../create/`
(`credito_create`) y `.../anular/<pk>/` (`credito_anular`) — `empleados/urls.py:94-97`. **No hay
ruta de borrado a propósito**: un crédito es un reconocimiento de deuda hacia el explorador, y
borrarlo haría desaparecer el rastro de quién lo concedió; se **anula**, igual que una sanción
...
2. **Vista y archivo.** `ProcesarSolicitudView` (`solicitudes/views/procesar_solicitud.py:15`).
...
1. **Método y ruta.** `POST /solicitudes/cancelar-solicitud/<int:solicitud_id>/` (`solicitudes/urls.py:100`, nombre `cancelar_solicitud`).
2. **Vista y archivo.** `CancelarSolicitudView` (`solicitudes/views/aprobacion_views.py:68`).
3. **Para qué existe.** Acción de cancelar del **solicitante**. Hace dos cosas distintas según el estado: si la solicitud está `pendiente` la retira en el acto; si está `aprobada` **no cancela nada**, registra la petición y deja la decisión al receptor (`solicitudes/use_cases/cancelar_solicitud.py:39-97`).
4. **Permiso.** `LoginRequiredMixin` y el usuario debe tener `empleado` (`:80-81`); el caso de uso comprueba que sea el solicitante (`cancelar_solicitud.py:70-71`).
5. **Entrada.** `solicitud_id` en la URL; `motivo` (str, **obligatorio**, no vacío tras `strip()`) en el cuerpo `POST` (`aprobacion_views.py:82`, vía `exigir_texto_json` de `core/utils/comentarios.py:44`).
6. **Validaciones, en orden.** (a0) `motivo` con contenido, si no **400 `comentario_requerido`** antes de tocar nada (`aprobacion_views.py:82-84`); (a) la solicitud es propia (`cancelar_solicitud.py:70`); (b) `bloquear_partes` toma el lock de las dos personas (`:79`); (c) si está `pendiente`, transiciona a `cancelada` y termina (`:81-88`); (d) si está `aprobada` entra en `_pedir_cancelacion` (`:227`): no hay ya una petición pendiente (`:237`) ni una terminal (`:243`), hay receptor registrado (`:246`), hay `fecha_resolucion` (`:250`), no han pasado más de **24 h** desde ella (`:254`, `VENTANA_PEDIR_CANCELACION_HORAS`), ninguna de las fechas está ya cumplida (`:263`), y pasan la **guardia LIFO** y la **de integridad** (`:267`).
7. **Respuesta 200.** Dos formas. Pendiente retirada: `json_ok({'message': msg, 'cancelada': True})` (`aprobacion_views.py:110`). Aprobada: `json_ok({'message': msg, 'cancelada': False, 'cancelacion_pendiente': True})` (`:114`). El frontend **debe** mirar `cancelada`: en el segundo caso el cambio sigue vigente.
8. **Errores.** `403 forbidden` (sin empleado o no es propia), `400 cambio_mas_reciente` (LIFO), `400 conflicto_integridad`, `400 ventana_expirada` (pasaron las 24 h desde la aprobación), `400 cancelacion_ya_pedida`, `400 cancelacion_cerrada` (rechazada o caducada: el cambio quedó firme), `400 invalid_state`, `500 internal_error` (`aprobacion_views.py:89-101,115-117`), y **`400 comentario_requerido`** con el texto `Escribe el motivo de la cancelación.` (`MSG_MOTIVO`, `core/utils/comentarios.py:25`) si el motivo llega vacío. El mapeo del resto se hace **inspeccionando el texto del mensaje**, no un código; `comentario_requerido` es el único que sale de una comprobación explícita.
9. **Efectos secundarios.** Si era pendiente: transición a `cancelada`, `fecha_resolucion` y `fecha_cancelacion` al mismo instante, notificación de cancelación (`cancelar_solicitud.py:81-88`, `aprobacion_views.py:107-109`). Si era aprobada: escribe **solo** los cuatro campos de la petición (`cancelacion_estado`, `..._solicitada_por`, `..._solicitada_en`, `..._motivo`, `:271-274`) y notifica al receptor. **No toca turnos ni deudas ni el `estado`.** En ambos casos invalida los contadores en caché (`aprobacion_views.py:117`, `_invalidar_contadores` en `:184`).
10. **Servicios.** `CancelarSolicitudUseCase.execute` → `_pedir_cancelacion`, `bloqueo_lifo` (`:423`), `bloqueo_integridad` (`:460`); después `CacheService.delete_many` y `NotificacionService.crear_notificacion_cancelacion` o `crear_notificacion_peticion_cancelacion` (`solicitudes/services/notificacion_service.py:631`).
11. **Tests.** `solicitudes/tests/test_cancelacion_lifo.py`: `test_pedir_cancelacion_no_cancela_nada` (`:150`), `test_no_se_puede_pedir_pasado_el_plazo_del_solicitante` (`:263`) y `test_pedir_cancelacion_sin_motivo_se_rechaza` (`:167`), que hace el POST sin `motivo` y exige 400 con `code == 'comentario_requerido'` sin que se abra la petición.
...
1. **Método y ruta.** `POST /solicitudes/cancelar-solicitud/<int:solicitud_id>/responder/` (`solicitudes/urls.py:102`, nombre `responder_cancelacion`).
2. **Vista y archivo.** `ResponderCancelacionView` (`solicitudes/views/aprobacion_views.py:120`).
...
4. **Permiso.** `LoginRequiredMixin` + `empleado` (`:131-132`); el caso de uso exige ser **el receptor** de esa solicitud, comparando contra `explorador_receptor_id` (`cancelar_solicitud.py:312-313`).
5. **Entrada.** `solicitud_id` en la URL; `accion` (str, obligatorio, `aprobar` o `rechazar`, `:134-137`); `comentario_respuesta` (str, **obligatorio**, `:139-141`).
6. **Validaciones, en orden.** (a) `accion` válida (`:135-137`); (a2) `comentario_respuesta` con contenido, si no **400 `comentario_requerido`** (`:139-141`); (b) el que responde es el receptor (`cancelar_solicitud.py:312`); (c) `cancelacion_estado == 'pendiente'` —si es terminal se explica por qué ya no se puede (`:315-318`)—; (d) la solicitud sigue en `aprobada` (`:320`); (e) **caducidad**: no han pasado más de 24 h desde `cancelacion_solicitada_en` (`:331`, `_caducar_si_vencida` en `:395`); (f) `bloquear_partes` toma el lock (`:337`); y **solo si aprueba**: (g) guardia LIFO y de integridad **otra vez** (`:359`), (h) ninguna fecha ya cumplida (`:363`).
7. **Respuesta 200.** `json_ok({'message': msg, 'aprobada': <bool>})` (`aprobacion_views.py:165`).
8. **Errores.** `403 forbidden` (sin empleado o no es el receptor), `400 accion_invalida`, `400 cambio_mas_reciente`, `400 conflicto_integridad`, `400 comentario_requerido` (comentario vacío, texto `MSG_COMENTARIO`), `400 plazo_vencido` (la petición caducó), `400 invalid_state`, `500 internal_error` (`:147-156,167-168`).
9. **Efectos secundarios.** **Rechazo:** escribe `cancelacion_estado='rechazada'` + quién y cuándo, y añade la nota al motivo; nada más (`cancelar_solicitud.py:340-350`). **Aprobación:** `_revertir_por_tipo` (`:646`) restaura el snapshot y cancela deudas, se refrescan desde BD los FK `turno_origen`/`turno_destino` que quedaron colgando tras borrar los turnos materializados (`:364-372`), transición a `cancelada`, `fecha_cancelacion = ahora` **sin tocar `fecha_resolucion`**, `cancelacion_estado='aprobada'` y anotación en `comentario` (`:379-389`). En ambos casos: invalidación de contadores y notificación al solicitante (`aprobacion_views.py:171-177`).
10. **Servicios.** `CancelarSolicitudUseCase.responder_cancelacion` (`:287`) → `bloquear_partes`, `bloqueo_lifo` (`:423`), `bloqueo_integridad` (`:460`), `_revertir_por_tipo` (`:646`) → servicio de aplicación del tipo → reconciliación; después `CacheService.delete_many` y `NotificacionService.crear_notificacion_respuesta_cancelacion` (`notificacion_service.py:665`).
11. **Tests.** `solicitudes/tests/test_cancelacion_lifo.py`: `test_solo_el_receptor_puede_responder` (`:200`), `test_receptor_rechaza_y_el_cambio_queda_firme` (`:209`), `test_receptor_aprueba_y_se_revierte` (`:229`), `test_la_peticion_caduca_si_el_receptor_no_responde` (`:240`) y `test_responder_cancelacion_sin_comentario_se_rechaza` (`:185`).
12. **Gotchas.** (a) Las guardias pueden **fallar aquí aunque pasaran al pedir**: en 24 h puede entrar otro cambio sobre los mismos días, y entonces el receptor recibe el error de algo que él no provocó. (b) La caducidad es **perezosa**: nada la marca en segundo plano, se reconoce al responder (`_caducar_si_vencida`) o se filtra al listar (`solicitudes/views/notificaciones_listas.py:173-197`). Una petición vencida no tiene efecto pendiente que aplicar, así que basta con reconocerla cuando alguien la mira. (c) Los tres estados terminales son **definitivos**: no se vuelve a `pendiente` ni se puede volver a pedir; la salida es un cambio nuevo o que un supervisor cancele desde Gestión (`cancelar_solicitud.py:415`).
...
2. **Vista y archivo.** `PermisoMediaJornadaCancelView` (`permisos/views.py:453`) y `PermisoMediaJornadaCancelResponderView` (`:521`).
3. **Para qué existe.** El mismo acuerdo que en solicitudes, aplicado al permiso de media jornada de temporada. Como el permiso **no tiene receptor**, la contraparte es el **supervisor**: es quien aprobó el permiso y quien responde por la cobertura del día (`permisos/models.py:121-125`).
...
10. **Servicios.** `AprobarAmbosRolesUseCase` → `SolicitudAprobacionService` → `SolicitudFactory.revalidar_para_aprobar` (`solicitudes/services/solicitud_factory.py:391,432`) → servicio de aplicación del tipo.
...
2. **Vista y archivo.** `CoberturaCandidatosView` (`solicitudes/views/api_fin_semana.py:192`).
...
12. **Gotchas.** El filtro (e) existe porque el desplegable y la validación de envío miraban fechas distintas: el selector solo el día de cesión y el backend además el día de pago, así que ofrecía compañeros que el envío tumbaba (`:258-263`, commit `9c429e3`). Es el patrón 37 de `PROTECTION_PATTERNS.md`. Si tocas este endpoint, toca `_validar_semana_cobertura` en el mismo commit. **Este endpoint no sabe nada del modo «2 compañeros»**: evalúa una jornada a la vez, así que la regla del día de pago libre no le corresponde y vive en `_pago_dos_debe_estar_libre` (§ 8.3). El campo `mi_jornada_pago` de la respuesta es lo que el formulario usa para adelantarla.

#### `GET /solicitudes/dfds-companeros/`

1. **Método y ruta.** `GET /solicitudes/dfds-companeros/` (`solicitudes/urls.py:114`, nombre `dfds_companeros`).
2. **Vista y archivo.** `DFDSCompanerosView` (`solicitudes/views/api_fin_semana.py:16`).
3. **Para qué existe.** Llena el selector de compañeros de los **dos** formularios de fin de semana: D FDS (cesión) y CAMBIO DESCANSO en su modalidad de finde (intercambio). Devuelve a **todos** los candidatos, disponibles o no, con el motivo: no se esconde a nadie, se explica por qué no puede (`solicitudes/views/api_fin_semana.py:18-20`).
4. **Permiso.** `LoginRequiredMixin`; sin `request.user.empleado` responde `{'companeros': []}` (`:38-40`).
5. **Entrada.** `fecha` (str `YYYY-MM-DD`, obligatorio, el día que cedes) y `tipo_solicitud_id` (int, opcional; por defecto D FDS, `:56`).
6. **Validaciones, en orden.** (a) `fecha` parseable, si no **400 `bad_request`** (`:42-44`); (b) la fecha es sábado o domingo, si no **400 `no_finde`** (`:45-46`); (c) **la estrategia del tipo implementa `EstrategiaFinDeSemana`**, si no **400 `tipo_sin_selector_finde`** (`:65-69`).
7. **Respuesta 200.** `{'success': true, 'companeros': [{'id', 'nombre', 'etiqueta', 'dia', 'dia_fecha', 'disponible', 'motivo'}], 'otro_dia': '<sábado|domingo>'}`, ordenada por disponibles primero y luego alfabética (`:89-90`).
8. **Errores.** `400 bad_request`, `400 no_finde`, `400 tipo_sin_selector_finde`.
9. **Efectos secundarios.** Ninguno: solo lee.
10. **Servicios que invoca.** `SolicitudFactory.get_strategy` y, sobre la estrategia, `disponibilidad_companero` y `etiqueta_companero` (`:79,85`), definidos en el mixin `solicitudes/services/strategies/estrategia_fin_de_semana.py:47`.
11. **Tests.** `solicitudes/tests/test_contrato_fin_de_semana.py`: `test_un_tipo_de_finde_contesta` (`:109`) y `test_un_tipo_que_no_es_de_finde_se_rechaza_en_vez_de_inventar` (`:116`).
12. **Gotchas.** La regla de disponibilidad **no es la misma en los dos formularios**: el intercambio (CAMBIO DESCANSO) exige que el compañero trabaje el **otro** día del finde para poder canjearlo; la cesión (D FDS) solo exige que tenga **libre** el día que recibe (`estrategia_fin_de_semana.py:64-66`). Y `etiqueta` solo viene rellena cuando `disponible` es `true` (`:85`).
...
|---|---|---|
| `solicitud_orchestrator.py` | Encadena los chequeos transversales —dedupe, cierre, sanciones, restricción— y despacha al tipo. Contiene además los dos flujos multi-solicitud: `_procesar_cobertura_dos` y `_procesar_doblada_permanente_multi`. El primero lleva delante `_pago_dos_debe_estar_libre`, el **único guard de pareja** del archivo: mira una condición que ninguna de las dos solicitudes puede ver por separado (ver § 8.3). Los dos envuelven su bucle en `EmailOutboxService.envio_agrupado()` **por fuera** del `atomic()` (`:278,530`): los 3N correos comparten conexión y, sobre todo, **nada se entrega hasta que el bloque termina, ya commitado** | `CrearSolicitudUseCase` |
...
| `permisos/credito_horas_service.py` | **Horas a favor del explorador.** `creditos_disponibles` (`:37`, filtra el saldo **en SQL** con `F()` en vez de recorrer el histórico en Python) y `saldo_disponible` (`:53`); `otorgar` (`:66`); `consumir` (`:100`) gasta en **FIFO** y comprueba el saldo **antes** de escribir nada, porque un consumo a medias dejaría créditos gastados contra un pago que la vista va a rechazar; `revertir_consumos` (`:152`) los devuelve al borrar el PDH; `anular` (`:176`) solo si no se consumió nada. `select_for_update` en el consumo: sin él, dos pagos simultáneos gastan el mismo crédito dos veces | `PDHCreateView`, `PDHDeleteView`, `ConsolidadoHorasService`, CRUD de crédito |
...
del mismo día, y `turnos/tests/test_reporte_dia_paridad.py` lo detecta.
...
No hay paquete `repositories/`: las consultas reutilizadas viven en el manager del modelo
(`solicitudes/models.py:136-149`). Ver § 3.4.
...
| `services/validators/doblada_validator.py` | Días especiales de doblada: los festivos de lunes a viernes y la temporada **sí** están permitidos (`:170-198`); el receptor que descansa el día de pago —casos 1.5/1.8 de la matriz— en `validar_receptor_tiene_jornada_en_fecha_pago` (`:272-294`), llamado desde `strategies/doblada_strategy.py:251`; y el caso del deudor que descansa el día de pago (`:296`) |
| `services/validators/doblada_flujo_validator.py` | Los **ocho tramos de flujo** de `DobladaStrategy.validar_solicitud`, extraídos de la estrategia: jornadas y coincidencia (`:115`), día no comprometido (`:204`), cesión en festivo (`:300`), reglas comunes (`:359`), pago en sábado (`:396`), cobertura en el pago (`:486`), intercambio (`:554`) y `validar_pago_en_sabado` (`:602`). No están en `doblada_validator.py` porque **devuelven** el mensaje en vez de lanzarlo: `validar_pago_en_sabado` devuelve un `RequiereCambioTurnoPrevio` —subclase de `str` con datos colgados— y viajar dentro de un `ValidationError` lo aplanaría a texto, dejando al formulario sin el recuadro «Ir a Cambio de Turno Sencillo» (el porqué, en el docstring del módulo, `:1-46`) |
| `services/solicitud_validator.py` | Validación de nivel de solicitud, por encima del tipo. Es la **fachada** que reúne las primitivas de base, CT y doblada; `doblada_flujo_validator` la importa a ella, no a `DobladaValidator` (`services/validators/doblada_flujo_validator.py:61-64`) |
...
| `verificar_crons` | **Comprueba desde fuera que los dos crons corren**: el del outbox y el de sanciones. Mide la **espera** del correo pendiente más antiguo que ya tocaba enviar, no el tamaño de la cola —una cola larga con worker vivo se vacía sola; una fila que lleva horas parada solo se explica porque nadie la recoge (`verificar_crons.py:131-174`)—. Cada problema sale con el marcador `CRON_NO_EJECUTADO` (`:69`) y el comando termina con código `1`, para que sea un *check* y no un informe que alguien tiene que acordarse de leer (`:124-127`). Opciones: `--umbral-outbox MINUTOS` y `--json` (`:89-100`) | cron de vigilancia |
| `alertar_crons` | **El mismo diagnóstico, y además avisa.** Importa la clase de `verificar_crons` y ejecuta sus tres chequeos —`_revisar_outbox`, `_revisar_correos_fallidos` y `_revisar_sanciones`— sin duplicar criterio (`alertar_crons.py:66-73,104-108`); si alguno falla, **manda el correo él mismo**. Existe porque `verificar_crons` solo *diagnostica*: en AWS bastaba —el `crontab` de Linux avisa por correo al salir con código distinto de cero y CloudWatch enganchaba los marcadores con un *metric filter*—, pero **Dokploy no tiene disparador para un Schedule que falla**: sus notificaciones cubren despliegues, backups, limpieza de Docker y umbrales del servidor (`:4-18`). Envía con `smtplib` directo y **no** con `django.core.mail` (`:205-242`), por dos motivos: el vigilante no puede compartir maquinaria con lo que vigila —todo correo de la aplicación pasa por `EmailOutbox`, y si lo averiado es el worker del outbox la alerta se quedaría en la cola que denuncia— y el [ADR 016](./03-arquitectura/adr/016-transporte-de-correo-y-fiabilidad.md) restringe `django.core.mail` a `email_outbox_service.py` (`:20-34`). Reutiliza la configuración `EMAIL_*` de settings: mismo transporte, sin pasar por la cola (`:214-235`). Destinatarios: `--para` o la variable de entorno `ALERTAS_CRON_EMAIL` (separadas por coma); **sin destinatarios falla ruidosamente** (`:159-169`) —no se añadió un ajuste a `settings.py` a propósito: es configuración de despliegue—. **Latido semanal**: los lunes manda un correo de «sin novedad» aunque todo esté bien (`:78-79,119-121`), la única defensa contra que deje de correr *él*. Termina con código `1` si hay problema, **incluido no haber podido enviar el aviso** (`:142-149,154-155`). `--dry-run` imprime el correo sin enviarlo —y sin exigir destinatarios—, pero conserva la salida `1` si hay problemas (`:130-137`). Opciones: `--para`, `--dry-run`, `--umbral-outbox`, `--umbral-fallidos`, `--sin-latido` (`:86-101`) | Schedule diario |
| `cancelar_deudas_fin_semana`, `cancelar_deudas_huerfanas` | Saneamiento de deudas | puntual |
| `reaplicar_doblada` (`:259`), `corregir_doblada_cesion_total` (`:73`) | Reparación manual. ⚠ **Llaman a `aplicar_doblada_pago` sin validar y sin pasar por la guardia LIFO**, y se ejecutan cuando el día ya está descuadrado — el momento de más riesgo. `verificar_doblada.py:384` sugiere el de reaplicación al detectar errores. Ver § 16.2 | puntual |
...
**`verificar_crons` y `alertar_crons` no son duplicados — no los fusiones.** El primero
*diagnostica*: imprime, admite `--json` y deja el veredicto en el código de salida, que es
lo que lo hace útil como paso de un despliegue. El segundo *entrega el aviso* cuando no hay
infraestructura que lo haga por él. La lógica está escrita **una sola vez**:
`alertar_crons` importa la clase de `verificar_crons` y llama a sus tres `staticmethod`
(`solicitudes/management/commands/alertar_crons.py:66-73,104-108`), así que un chequeo nuevo
se añade en `verificar_crons` y los dos lo heredan. Lo que `alertar_crons` pone encima
—destinatarios, transporte, latido y código de salida— lo cubren 16 pruebas en
`solicitudes/tests/test_alertar_crons.py` (`:2-13`). Los Schedules y la configuración paso a
paso están en
[CHECKLIST_DESPLIEGUE_DOKPLOY.md](./05-referencia/deployment/01-dokploy/CHECKLIST_DESPLIEGUE_DOKPLOY.md)
(FASE 9 y § 11.2).
...
(`solicitudes/services/solicitud_aprobacion_service.py:124`) y, si falla, quien lee el mensaje es
...
`return` de fallo es hoy (`solicitud_aprobacion_service.py:134-138`):
...
(`solicitudes/services/doblada_pago_service.py:146-152`).
...
`rechazada` y `caducada` (`core/constants.py:167-195`). Los tres últimos son **terminales**:
...
(`solicitudes/use_cases/cancelar_solicitud.py:390`) y se filtra al listar
(`solicitudes/views/notificaciones_listas.py:173-197`).
...
| **Cobertura de día completo (2 compañeros): el día de pago tiene que estar LIBRE.** Las dos solicitudes comparten una única `fecha_pago`, así que ese día se devuelven las **dos** medias jornadas —la AM a un compañero y la PM al otro— y el solicitante termina en **AM+PM**. Eso solo cabe partiendo de cero: con media jornada propia solo alcanza para pagarle a uno y el otro habría cubierto gratis. **Es un guard de PAREJA, y por eso no vive en la strategy:** `_validar_semana_cobertura` valida cada solicitud por separado y no puede ver el conjunto. Rechazaba el caso igual, pero por la mitad equivocada del problema (`tomadas & pre_sol_fp`, la jornada que choca) y con un mensaje que manda a hacer un **Cambio de Turno sencillo** — que aquí es un **callejón sin salida**: girar la jornada propia traslada el choque al otro compañero. Y si la `fecha_pago` fuera de mantenimiento, ese día ni siquiera se puede pedir un CT. El guard corta el par **antes** de validar ninguna de las dos y da el motivo real, con las dos salidas que sí existen: el día libre de la semana, o cobertura de **solo AM** / **solo PM** | `SolicitudOrchestrator._pago_dos_debe_estar_libre`, llamado al inicio de `_procesar_cobertura_dos` (justo tras la restricción médica, antes de construir `datos_am`/`datos_pm`). Se apoya en `CambioDescansoAplicacionService._jornadas_actuales`: **estado real del día**, no la jornada base (P-jornada-real). El formulario lo adelanta con `mi_jornada_pago`, que ya devuelve `GET /solicitudes/cobertura-candidatos/`: recuadro rojo bloqueante `cob-aviso-dos-libre` + error al enviar, y en modo `DOS` se **suprime** el aviso de «Ir a Cambio de Turno» (`static/js/cambio-turno/solicitar_cambio_descanso.js`, `mostrarAvisoDosDiaLibre` y la salida temprana de `verificarPagoCobertura`). El desplegable de día de pago además **deshabilita los días de mantenimiento**: ese día no opera nadie, así que no hay jornada del compañero que cubrir. El texto del rechazo y el del aviso del formulario son un **espejo** sincronizado a mano (`solicitud_orchestrator.py:228-234` ↔ `solicitar_cambio_descanso.js:1091-1095`), con centinela `AvisoDiaPagoLibreEspejoTest`. Tests: `solicitudes/tests/test_cobertura_dos_companeros.py:29` (espejo), `:154` (`test_dia_de_pago_con_media_jornada_propia_es_rechazado`, que además fija que el mensaje **no** mande al CT) y `:174` (`test_dia_de_pago_donde_ya_doblo_tampoco_sirve_para_los_dos`) | Sin el guard el rechazo llega igual, pero con el consejo equivocado: el usuario hace el Cambio de Turno, vuelve, y choca con el otro compañero. Si alguien "arregla" el síntoma relajando `_validar_semana_cobertura`, se crea media cobertura pagada y la otra media gratis |
| **Una sub-modalidad de CAMBIO DESCANSO entre semana se apaga desde el backend, no desde el formulario.** Hoy está apagada `cambio_doblada`. El interruptor es la constante `SUBMODALIDADES_SEMANA_DESHABILITADAS` y se comprueba dentro de `validar_solicitud`, en la rama de ENTRE SEMANA y **antes** de despachar a ningún sub-validador. Como `revalidar_para_aprobar` termina llamando a `validar_solicitud`, el apagado cubre las dos entradas: una solicitud que se hubiera colado antes del despliegue **tampoco se puede aprobar**. El código del sub-flujo **no se borra** (`_validar_semana_cambio_doblada` y su aplicación siguen enteras): reactivar es sacar la clave del `frozenset` y quitar la clase `disabled` de la tarjeta | Constante: `solicitudes/services/strategies/cambio_descanso_strategy.py:53` (motivo comentado en `:40-52`); comprobación y mensaje en `:248-256`, con la etiqueta sacada de `DobladaDetalle.SUBMODALIDAD_SEMANA_CHOICES` (`:252`) para que el texto siga siendo cierto al apagar o reactivar otra. Doble entrada: `solicitudes/services/strategies/base_strategy.py:114`. Reflejo visual (**solo cosmético**): `templates/solicitudes/solicitar_cambio_descanso.html:128-133`. Tests: `solicitudes/tests/test_cambio_doblada_deshabilitado.py:74,80,93,120,131`. Detalle en [05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md](./05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md) | Si el apagado viviera solo en el template, bastaría con quitar la clase desde DevTools —o tener la pestaña abierta desde antes del despliegue, con el HTML y el JS anteriores— para que el POST entrara. Y si la comprobación estuviera en la vista y no en `validar_solicitud`, una solicitud creada antes del apagado se seguiría aprobando |
...
| **Cumplir la auto-sanción salda la deuda del mes que la originó.** Pasa a `consumida_por_sancion` con FK a la sanción: el castigo ES el pago | `solicitudes/services/deuda_corporativa_service.py:114` (`ES_AUTOMATICA` y `levantada_en__isnull=True` en `:137-139`) | El explorador termina sus 15 días con el mismo saldo vencido que lo metió en ellos y vuelve a ser sancionado por lo mismo: un bucle del que no se sale |
| **Levantar una auto-sanción por deuda CONDONA la deuda de ese mes.** Pasa a `'condonada'` —estado propio, ni `cancelada` (corrección técnica) ni `consumida_por_sancion` (esa la paga el castigo cumplido)— con `sancion_consumidora` y `fecha_consumo`. Solo aplica a auto-sanciones —reconocidas por el criterio único `ES_AUTOMATICA`, ver la regla siguiente—, solo al mes que la originó, y es **idempotente**. Se dispara desde `SancionEmpleado.levantar()` y **no desde la vista**, dentro de su misma transacción: es el único punto de entrada al levantamiento, para que ninguna vía futura (comando, admin, proceso automático) pueda olvidarse. **Lo que NO se perdona es el expediente:** `_periodos_ya_evaluados` y `_antecedente` siguen viendo la sanción levantada, así que la reincidencia se mide igual. Razonado en el [ADR 013](./03-arquitectura/adr/013-condonacion-al-levantar-la-sancion.md) | Servicio: `solicitudes/services/deuda_corporativa_service.py:181` (guardas en `:212-215`, marca y `update` en `:218-236`, roll-up del permiso en `:241-249`). Disparador: `empleados/models.py:409` (`levantar`), llamada en `:438`. Aviso **previo** al supervisor (estimación): `empleados/views/sanciones.py:149`, `:289`; el número **posterior** sale de `horas_condonadas_por` (`deuda_corporativa_service.py:258`), que lee lo ya escrito, y lo usan tanto el mensaje al supervisor (`sanciones.py:316`) como el aviso por campana al explorador (`notificar_condonacion`, `:282`, dentro de la misma transacción, `empleados/models.py:442-443`), que usa el tipo **propio** `'sancion_levantada'` (`:330`, declarado en `solicitudes/models.py:22` con su motivo en `:18-21`) para que la campana no pinte el triángulo rojo de la sanción sobre la noticia contraria (`templates/solicitudes/notificaciones_list.html:65-68`). Antecedente intacto: `deuda_corporativa_service.py:369` (`_periodos_ya_evaluados`) y `:340` (`_antecedente`). Tests: `solicitudes/tests/test_consumo_deuda_por_sancion.py:145,174,192,208,226,297,318,339,362,385` y `solicitudes/tests/test_sancion_automatica_deuda.py:257` | Es el bug que cierra: la deuda quedaba en un **limbo permanente**. El mes ya venció, así que no se puede pagar (`Periodo.esta_vencido`, `solicitudes/services/sancion_deuda_calculo.py:99`); esa sanción no la consumirá al llegar a su fin, porque el consumo exige `levantada_en IS NULL`; y no nace otra que lo haga, porque el mes ya cuenta como juzgado. Horas activas para siempre, ni cobrables ni saldables |
| **Qué sanción es automática lo dice su periodo, nunca el texto del motivo.** `ES_AUTOMATICA = Q(periodo_anio__isnull=False)` y `ES_MANUAL = Q(periodo_anio__isnull=True)` son el criterio **único** de las siete consultas que deciden algo: consumo, antecedente, meses ya juzgados, contador de pendientes, las tres de auditoría y la del barrido. El prefijo `[AUTO-DEUDA]` se sigue escribiendo en el motivo, pero es **etiqueta de lectura**: ninguna decisión depende de él | Constantes y motivo en `solicitudes/services/deuda_corporativa_service.py:58-69`; usos en `:137` (consumo), `:386` (meses ya juzgados), `:574` (contador de pendientes), `:731`, `:740` y `:777` (auditoría) y `:828` (barrido); el prefijo se escribe en `:497` | El motivo es **editable** desde la pantalla de sanciones. Con la detección por texto, retocar la redacción volvía la sanción invisible para las siete consultas a la vez: su deuda no se consumía al cumplirse el castigo **y** su mes dejaba de contar como juzgado, así que el sistema podía sancionar por segunda vez un mes ya castigado. Lo fijan `test_condona_aunque_se_haya_editado_el_motivo` y `test_editar_el_motivo_no_reabre_el_ciclo_de_sanciones` (`solicitudes/tests/test_consumo_deuda_por_sancion.py:318`, `:339`) |
| **Una deuda ya extinguida por una sanción no se recalcula ni resucita.** `'condonada'` cuenta como hecho ocurrido igual que `'pagada'` y `'consumida_por_sancion'` | `permisos/deuda_permiso_service.py:44` (`_tiene_historia`, `:54-55`) y el revert de PDH en `permisos/pago_horas_service.py:387` | Se reabre por la puerta de atrás una deuda que alguien decidió cerrar, y con ella la posibilidad de volver a sancionar por lo ya perdonado |
...
| Un PDH pide entre 0 y 24 horas | `permisos/models.py:53-58` | Pago imposible |
| Un crédito de horas a favor pide entre 1 minuto y 24 h | `permisos/models.py:389-392` (validadores **de campo**, no `clean()`) | Crédito imposible |
| **El crédito aplicado a un PDH no puede superar las horas que ese pago salda** | `permisos/credito_horas_service.py:128-131` | Doble descuento: el consolidado restaría dos veces las mismas horas |
| **Las horas a favor NO intervienen en la sanción.** Un mes vencido sigue sancionando aunque se salde con crédito: lo sancionable es haber dejado vencer el plazo, no el importe | `solicitudes/services/deuda_corporativa_service.py` **no consulta** `CreditoHoras`; el centinela es `permisos/tests/test_credito_horas.py:222` | Una bolsa de crédito volvería inmune a la sanción a quien nunca paga a tiempo |
| El permiso `MEDIA_JORNADA_TEMPORADA` no genera deuda (`tiempo=0`) | `permisos/models.py:113-117` y `permisos/views.py:855` | Deuda duplicada |
...
| Visibilidad del menú | Catálogo de **sesiones** por empleado: cada `Sesion` agrupa nombres de URL y se enciende o apaga por persona | `core/sesiones.py` |

El último renglón no es un permiso más. `core/sesiones.py` es un **catálogo declarativo** que
asocia cada entrada de menú con los nombres de URL que habilita
(`Sesion(clave, etiqueta, grupo, [urls], defecto_explorador=…)`); apagar una sesión oculta la
entrada y corta el acceso a sus rutas. Dos detalles con trampa, advertidos en el propio archivo:
`sanciones_list` y `restricciones_list` quedan **fuera** de sus sesiones a propósito —son
pantallas compartidas que al explorador le enseñan solo lo suyo, y bloquearlas dejaría al
supervisor sin ver las propias—, y quien administra roles puede concederse cualquier permiso, así
que esa sesión es la llave maestra. `is_staff` no se recorta por este mecanismo. La sesión
`credito_horas` va **separada** de `pdh`: reconocer horas compromete a la corporación, mientras
que un PDH solo constata lo que el explorador ya debía.
...
| Caducidad | `max_age` = `APPROVAL_LINK_MAX_AGE_DAYS` × 86400, por defecto 30 días | `tokens_aprobacion.py:51-54`, `config/settings.py:262` |
...
| Vínculo con la persona | El token solo vale para quien **hoy** ocupa el rol; si al explorador le cambian de supervisor, el enlace del anterior deja de servir | `tokens_aprobacion.py:67-71,96-107` |
| Fallo cerrado | Firma inválida, token caducado, de otra solicitud o de otro rol → `False`, nunca excepción | `tokens_aprobacion.py:76-107` |
...
| `aws secretsmanager create-secret --name swalp/SECRET_KEY` | *(era del checklist de Fargate, descartada y borrada el 2026-09-04)* | En el plan EC2 el secreto va en el `.env` de la instancia; ver [checklist EC2+RDS](./05-referencia/deployment/99-aws/CHECKLIST_DESPLIEGUE_AWS_RDS.md) FASE 4 |
| Instancia única con Elastic IP, sin ALB, con un `.env` en la máquina | [checklist EC2+RDS](./05-referencia/deployment/99-aws/CHECKLIST_DESPLIEGUE_AWS_RDS.md) | Sin reparto de tráfico no hay nada que pueda divergir |
...
| `CORS_ALLOWED_ORIGINS` | de `env.list`, localhost por defecto | `config/settings.py:350-353` |
| `CORS_ALLOW_CREDENTIALS` | `True` | `config/settings.py:335` |
| `AUTH_PASSWORD_VALIDATORS` | los 4 estándar de Django | `config/settings.py:204-209` |
...
(`solicitudes/services/solicitud_orchestrator.py:615`) y
`logger.warning('Cobertura con 2 compañeros revertida (solicitante=%s): %s', solicitante.id, e)`
(`:273-274`). Mantén ese criterio: identificadores sí; motivos médicos, contraseñas y tokens,
nunca.
...
(`solicitudes/views/procesar_solicitud.py:32-34`).
...
| `csrf_failure` | `core/errors.py:153-163` | `CSRF_FAILURE_VIEW` (`config/settings.py:445`) |
...
externo: CSS y SVG en línea (`templates/errors/_base_error.html:7-30`).
...
(`core/errors.py:153-163`): decirle a un atacante si falló por «CSRF token missing» o por «Referer
...
(`config/settings.py:23`). Se lee con `django-environ` desde `.env`
(`config/settings.py:11,21,25`); el porqué está en
...
5. Regístrala en `SolicitudFactory` (`services/solicitud_factory.py:131-195`). Si cae en la
...
6. Si genera o revierte turnos, crea `services/<tipo>_aplicacion_service.py` con snapshot
   ...
   (`use_cases/cancelar_solicitud.py:646`).
...
2. Localiza la estrategia con `SolicitudFactory.get_strategy`
   (`services/solicitud_factory.py:131`). Si cayó en la estrategia por defecto hay un
...
5. Si el desplegable ofrece a alguien que el envío rechaza, es el patrón 37: selector y
   validación miran fechas distintas (`solicitudes/views/api_fin_semana.py:267-274`).
...
> [arquitectura-aws-rds-recomendada.md](./05-referencia/deployment/99-aws/arquitectura-aws-rds-recomendada.md)
> §5 y §8.
...
**Paso a paso:** [CHECKLIST_DESPLIEGUE_AWS_RDS.md](./05-referencia/deployment/99-aws/CHECKLIST_DESPLIEGUE_AWS_RDS.md)
(FASES 0-11) y [MANUAL_DESPLIEGUE_EC2.md](./05-referencia/deployment/99-aws/MANUAL_DESPLIEGUE_EC2.md).
...
[CHECKLIST_DESPLIEGUE_AWS_RDS.md](./05-referencia/deployment/99-aws/CHECKLIST_DESPLIEGUE_AWS_RDS.md) y
[CONFIGURACION_PRODUCCION.md](./05-referencia/deployment/03-operacion/CONFIGURACION_PRODUCCION.md).
...
| **Los crons no corren** | Dos comandos, dos trabajos. `python manage.py verificar_crons` (`--json` para consumo automático) **diagnostica**: comprueba el del outbox y el de sanciones, cada problema sale marcado con **`CRON_NO_EJECUTADO`** o **`CORREOS_FALLIDOS`** y termina con código `1` (`solicitudes/management/commands/verificar_crons.py:69-70,124-127`). `python manage.py alertar_crons` corre esos mismos tres chequeos y **manda el correo él mismo** (`solicitudes/management/commands/alertar_crons.py:104-108`), porque Dokploy no dispara notificación cuando un Schedule falla: destinatarios en `--para` o en la variable de entorno **`ALERTAS_CRON_EMAIL`** —sin ninguno falla ruidosamente (`:159-169`)— y transporte `smtplib` directo, al margen del outbox que vigila (`:205-242`). **Los lunes manda un latido de «sin novedad» aunque todo esté bien** (`:119-121`): un vigilante mudo y uno muerto se ven igual desde fuera. Probarlo con `--dry-run`, que imprime el correo sin enviarlo (`:130-137`) |
...
cierre** y solo queda un `logger.critical` (`solicitudes/services/solicitud_orchestrator.py:152-160`).
...
**Dos banderas propias, para los tests que caducan.** Ninguna corre por defecto:

| Bandera | Qué hace | Cuándo se lanza |
|---|---|---|
| `--dias-en-el-futuro=N` | Corre la suite entera como si hoy fuese dentro de N días (`conftest.py:67-71`) | Antes de un despliegue, o al tocar sanciones, deudas y vencimientos |
| `--anio-planificado` | Finge que el checklist de apertura de año está completo, parcheando `AperturaAnioService.completo` (`conftest.py:104,111-113`) | Ensayo de diciembre, junto con `--dias-en-el-futuro=90` |

Existen porque la suite tiene una clase de fallo que el CI no ve: el test que fija en el código
una fecha que solo es válida "por ahora". Pasa en verde durante semanas y revienta un día
cualquiera sin que nadie haya tocado nada; ya ocurrió tres veces a la vez, con tres causas
distintas (`conftest.py:11-30`). **Los fallos que produce `--dias-en-el-futuro` son el hallazgo,
no un error de la corrida**, y el arreglo correcto es inyectar `hoy` en el servicio, no congelar
el reloj (`conftest.py:38-42`). El procedimiento completo está en
[05-referencia/deployment/MANUAL_TESTS_QUE_CADUCAN.md](./05-referencia/deployment/03-operacion/MANUAL_TESTS_QUE_CADUCAN.md).

### 14.2 Cómo está partida la suite

El módulo de horas a favor añade `permisos/tests/test_credito_horas.py` (23 casos en 7 clases).
Dos son **centinelas de política**, no de código:
`test_el_credito_no_evita_la_sancion_de_un_mes_vencido` (`:222`) fija que el crédito no exime de
la sanción, y `test_el_minimo_del_campo_esta_alineado_con_el_paso` (`:322`) impide que vuelva el
desfase de `min` / `step` descrito en § 16.2. El resto cubre el FIFO, el reparto entre varios
créditos, la reversión al borrar el PDH y el caso que originó el módulo.
...
comentario o el motivo (`test_cancelacion_lifo.py:167` y `:185`;
...
| [011](./03-arquitectura/adr/011-sancion-independiente-del-pago.md) | La sanción por deuda se cumple entera, y la deuda se mide por meses | Pagar dejó de levantar la sanción (era una fianza reembolsable), la unidad sancionable pasa a ser `(explorador, año, mes)` alimentada por dobladas **y** permisos, cumplir el castigo extingue la deuda de ese mes y el nivel de reincidencia prescribe a los 45 días. Su punto 3 queda **enmendado por el 013**. Ver § 8.3 |
| [013](./03-arquitectura/adr/013-condonacion-al-levantar-la-sancion.md) | Levantar una auto-sanción por deuda condona la deuda del mes que la originó | La deuda de una sanción levantada quedaba en un limbo permanente —ni cobrable, ni pagable, ni extinguible—. Ahora pasa a `'condonada'` desde el propio `levantar()`. Contrapartida asumida: levantar cuesta horas reales y no se deshace; se avisa al supervisor **antes** de confirmar. Ver § 5.2 y § 8.3 |
...
`PROTECTION_PATTERNS.md` (raíz del repositorio) recoge **43 patrones** aplicados, cada uno con
...
| 39 | **La guardia vive donde está la ESCRITURA, no solo en la estrategia** (`PROTECTION_PATTERNS.md:1651`) | Todo `*_aplicacion_service` / `*_pago_service` valida sus precondiciones antes de la primera escritura, aunque ya estén validadas aguas arriba; **lanza, no arregla**; y su mensaje no da instrucciones de rol. Ampliado con la auditoría de esta rama: la **puerta abierta son los comandos de gestión**, el cierre de `aplicar_doblada_pago` es **parcial** y con su motivo, el principio "**una guardia que consulta el estado posterior no sirve en un método cuyo efecto es cambiar ese estado**", y el aviso de **no mover** la guardia de `jornada_cedida` al inicio del método |
| 40 | **Una reparación no aborta por una pieza que no encaja** (`PROTECTION_PATTERNS.md:1732`) | Un proceso de *reparación* captura el fallo de negocio de cada pieza, lo registra y sigue; uno de *validación* falla cerrado. Ver P4b y `reconciliar_dobladas_aprobadas` |
| 41 | **Un nombre privado consumido desde fuera es un contrato falso** (`PROTECTION_PATTERNS.md:1787`) | Si otro módulo importa tu `_nombre`, o lo renombras a público o dejas de exportarlo. No hay tercera opción |
| 42 | **Trinquete: la deuda conocida se congela y solo puede menguar** (`PROTECTION_PATTERNS.md:1846`) | La base conocida se declara en un test, no en un documento: así no puede crecer sin que alguien lo vea |
| 43 | **Quien decide sobre una operación no puede ser candidato de esa operación** (`PROTECTION_PATTERNS.md:1896`) | El supervisor aprueba los cambios de turno; queda fuera de los desplegables de compañero de los seis formularios |
...
sábado y **ya llevan el guard** (`doblada_pago_service.py:302-309` y `:536-543`). `cesion_parcial`
...
`reaplicar_doblada` (`solicitudes/management/commands/reaplicar_doblada.py:259`) y
...
El archivo tiene además una matriz de patrones por flujo (`PROTECTION_PATTERNS.md:1151`), un
...
### El aviso del formulario y el rechazo del servidor dicen la misma regla con las mismas palabras

*(Trabajo en curso: `solicitudes/services/solicitud_orchestrator.py`, `static/js/cambio-turno/solicitar_cambio_descanso.js` y su suite están en el árbol, sin confirmar.)*

La regla del día de pago libre en la cobertura con dos compañeros (§ 8.3) tenía **dos redacciones**: una en el aviso bloqueante del formulario y otra en el rechazo del POST. Es la misma regla, y el usuario puede toparse con ella por cualquiera de los dos caminos.

| Cambio | Impacto |
|---|---|
| El servidor adopta la redacción del formulario (`solicitud_orchestrator.py:228-234`) | Un solo texto para una sola regla, se llegue por donde se llegue |
| Había un **tercer** sitio con la misma regla en otra redacción —el bullet de la lista «Faltan datos» al pulsar Enviar—, que el centinela no veía porque vive en el mismo archivo JS que el recuadro rojo. Se unificó **por construcción**: tres constantes en `solicitar_cambio_descanso.js:1064-1067` (`DOS_LIBRE_TITULAR`, `DOS_LIBRE_MOTIVO`, `DOS_LIBRE_SALIDA`) que consumen el recuadro (`:1091-1095`) y el bullet (`:1291-1296`) | Dentro del front el literal existe **una sola vez**, así que los dos avisos no pueden separarse sin tocar la constante: garantía más fuerte que un test, y por eso ahí **no hay test**. La sincronización manual y el centinela solo hacen falta en el cruce **front ↔ servidor**, que son lenguajes distintos y no pueden compartir el literal |
| La única diferencia que queda es **deliberada**: el formulario nombra el día libre concreto (`solicitar_cambio_descanso.js:1075-1083`), el servidor no | El servidor no consulta el desplegable, así que no puede sugerir un día; y el formulario solo lo nombra si sigue siendo una opción viva |
| En el JS las negritas envuelven **frases completas** (`:1091-1095`) | Si partieran una frase por dentro, el `<strong>` quedaría en medio y la frase núcleo dejaría de ser localizable para el centinela |
| Los dos fuentes llevan un comentario que apunta al otro (`solicitud_orchestrator.py:222-227`, `solicitar_cambio_descanso.js:1085-1090`) | La sincronización es **manual**: quien edite un lado tiene el otro señalado en la misma pantalla |
| `AvisoDiaPagoLibreEspejoTest` (`solicitudes/tests/test_cobertura_dos_companeros.py:29`, caso `:60`) | Lee los dos fuentes y exige las **tres frases núcleo** literales (`:44-49`). Las sigue encontrando porque ahora viven en las constantes, y la costura de `_COSTURA` (`:54`) une los dos trozos de `DOS_LIBRE_MOTIVO`. Si falla, se ajusta el lado que quedó atrás; no se relaja la comparación |

Detalle y textos literales en [05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md](./05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md) (§ 6, cobertura con DOS compañeros).

### El aviso de doblada permanente deja de dar por firme lo que solo está pendiente

*(Trabajo en curso: los cambios de `solicitudes/services/strategies/doblada_permanente_strategy.py` y de su suite están en el árbol, todavía sin confirmar.)*

La **regla no cambia**: una doblada permanente del compañero, pendiente **o** aprobada, que comparta alguna fecha sigue bloqueando el nuevo acuerdo. Lo que cambia es el aviso, que era uno solo y decía «ya tiene una doblada permanente en esas fechas» incluso cuando el otro acuerdo estaba solo **pendiente de aprobación**: daba por firme lo que aún podía cancelarse.

| Cambio | Impacto |
|---|---|
| Tres mensajes en lugar de uno, ramificando por `det.solicitud.estado` y —si es `pendiente`— por `det.solicitud.explorador_solicitante_id` (`doblada_permanente_strategy.py:278-297`) | El usuario sabe si lo que le bloquea es un acuerdo cerrado o uno que todavía se puede caer, y actúa en consecuencia |
| La variante «la pendiente es tuya»: "Ya enviaste una doblada permanente a {compañero}…" (`:283-288`) | Es el caso frecuente. Mandarle a «elegir otro compañero» cuando la pendiente es **suya** lo despista: lo que tiene que hacer es esperar su propia solicitud |
| El listado de fechas del choque (`detalle`, `:267-269`) se mantiene en las tres variantes | El aviso sigue señalando la fecha exacta que bloquea |
| La rama compara `str(det.solicitud.estado or '').lower() == 'pendiente'` (`:278`), no `== 'pendiente'` a secas | El filtro que trae `otros` va por ORM y **MySQL no distingue mayúsculas**: una fila guardada como `'PENDIENTE'` llegaba hasta aquí y, comparada en Python tal cual, se anunciaba como **aprobada** — el error exacto que los tres mensajes vienen a corregir (motivo en `:274-277`) |
| `solicitudes/tests/test_doblada_permanente.py::DobladaPermanenteOtroAcuerdoTest`: `test_el_aviso_distingue_pendiente_de_aprobada` (`:355`) y `test_si_la_pendiente_es_propia_el_aviso_lo_dice` (`:371`) | El primero fija que el aviso de una pendiente **no** contiene "aprobada"; el segundo, que el de la propia empieza por "Ya enviaste" |

Mensajes literales y motivo en [05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md](./05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md) (§ 5, regla 9).

### «Cambio de doblada» queda deshabilitado, y el interruptor es del backend

*(Trabajo en curso: `solicitudes/services/strategies/cambio_descanso_strategy.py`, la tarjeta del template y `solicitudes/tests/test_cambio_doblada_deshabilitado.py` están en el árbol, sin confirmar.)*

| Cambio | Impacto |
|---|---|
| `SUBMODALIDADES_SEMANA_DESHABILITADAS = frozenset({'cambio_doblada'})` (`cambio_descanso_strategy.py:53`) | Un conjunto, no un `if` con un literal: apagar o reactivar otra sub-modalidad es editar una línea, y el test `test_el_interruptor_es_el_conjunto_y_no_un_literal_suelto` (`:131` del archivo de tests) lo vigila |
| La comprobación va en `validar_solicitud`, en la rama de ENTRE SEMANA y antes de despachar (`:248-256`) | Cubre creación **y** aprobación: `revalidar_para_aprobar` pasa por ahí (`base_strategy.py:114`), así que una solicitud colada antes del apagado tampoco se aprueba |
| El mensaje nombra la opción apagada con la etiqueta de `SUBMODALIDAD_SEMANA_CHOICES` (`:252`) y ofrece salida: «Elige otra de las opciones de esa semana» | El texto sigue siendo verdad si mañana se apaga otra, y el usuario no queda atascado sin saber qué hacer |
| La clase `disabled` de la tarjeta (`templates/solicitudes/solicitar_cambio_descanso.html:128-133`) es **solo el reflejo visual** | Se quita desde DevTools, y una pestaña abierta desde antes del despliegue conserva el HTML y el JS viejos: el POST llega igual al servidor. Por eso la defensa está en el backend |
| El código del sub-flujo **no se elimina** | `_validar_semana_cambio_doblada` y su aplicación siguen enteras. Reactivar = sacar la clave del `frozenset` + quitar la clase `disabled` |

Regla en § 8.3; sub-modalidades y mecanismo completo en [05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md](./05-referencia/solicitudes/REGLAS_NEGOCIO_SOLICITUDES.md) (§ 6, modalidad ENTRE SEMANA).

### El vigilante de las tareas programadas ahora avisa por su cuenta

*(Trabajo en curso: `solicitudes/management/commands/alertar_crons.py` y su suite están en
el árbol, todavía sin confirmar. El marcador `CORREOS_FALLIDOS` que este comando
reenvía sí está commitado, en `0b8d896`.)*

| Cambio | Impacto |
|---|---|
| **Nuevo `python manage.py alertar_crons`.** Ejecuta los tres chequeos de `verificar_crons` reutilizando su clase (`alertar_crons.py:66-73,104-108`) y, si alguno falla, envía el correo de alerta | `verificar_crons` solo diagnostica. En AWS bastaba: el `crontab` de Linux avisa por correo al salir con código distinto de cero y CloudWatch enganchaba los marcadores con un *metric filter*. **En Dokploy no existe ninguna de las dos cosas** —sus notificaciones cubren despliegues, backups, limpieza de Docker y umbrales del servidor, pero no hay disparador para un Schedule que falla (`:4-18`)— |
| **Transporte `smtplib` directo, no `django.core.mail`** (`:205-242`) | Dos motivos que apuntan al mismo sitio: el vigilante no puede compartir maquinaria con lo que vigila —si lo averiado es el worker del outbox, una alerta encolada se quedaría esperando junto a los correos que denuncia— y el [ADR 016](./03-arquitectura/adr/016-transporte-de-correo-y-fiabilidad.md) fija que fuera de los tests `django.core.mail` solo puede aparecer en `email_outbox_service.py`, así que un import aquí rompería ese invariante en silencio (`:20-34`). Se reutiliza la configuración `EMAIL_*` de settings: mismo transporte, sin cola (`:214-235`) |
| **Destinatarios por `--para` o `ALERTAS_CRON_EMAIL`**, sin ajuste nuevo en `settings.py` (`:159-169`) | Es configuración de despliegue y en Dokploy vive junto al resto de variables de la aplicación. Sin destinatarios el comando **falla ruidosamente**: un vigilante mal configurado que calla es peor que ninguno |
| **Latido semanal los lunes** (`:78-79,119-121`) | Única defensa contra que deje de correr *este* comando: un vigilante mudo y uno muerto se ven igual desde fuera. Semanal y no diario a propósito —un aviso diario de que no pasa nada se deja de leer en dos semanas, y entonces el latido no vale nada— |
| **Código de salida `1` también cuando el aviso no se pudo enviar** (`:142-149`) | El Schedule queda en rojo además de haber avisado. No se relanza la excepción: se informa por stderr para que el traceback no tape el diagnóstico, que es lo que alguien va a leer |
| **El cuerpo del correo lleva un pie con qué hacer ante cada marcador** (`:191-202`) | `CRON_NO_EJECUTADO` → revisar los Schedules de Dokploy (`swalp-outbox`, `swalp-sanciones`); `CORREOS_FALLIDOS` → `/admin/solicitudes/emailoutbox/`, filtrar por «fallido» y decidir |
| **Pruebas**: `solicitudes/tests/test_alertar_crons.py`, 16 casos | Prueban solo lo que este comando añade encima de `verificar_crons` —silencio cuando todo va bien, aviso, destinatarios, no dejar fila en el outbox, fallo de envío, latido y `--dry-run`—, para que los criterios de los chequeos no queden duplicados en dos suites (`:2-13`) |

División de trabajo entre los dos comandos en § 7.3; operación en § 13.5.
...
`980fe17`. Hasta aquí el sistema de horas era **unidireccional**: solo modelaba lo que el
explorador debe a la corporación, y `DeudaPermisoMes.minutos_pendientes` lo decía literalmente
—*"Nunca negativo: un exceso no genera crédito"* (`permisos/models.py:303-305`)—. Pero el caso
contrario ocurre: si su jornada es de 08:00 a 14:00 y el supervisor le pide entrar a las 07:00,
esa hora se la deben, y sin sitio donde anotarla se perdía en cuanto se saldaba la deuda menor
que tuviera encima.

Modelos nuevos `CreditoHoras` y `ConsumoCreditoHoras` (`permisos/models.py:355,435`), servicio
`CreditoHorasService` (`permisos/credito_horas_service.py`) y CRUD propio
(`empleados/views/credito.py`, `empleados/urls.py:94-97`, sesión `credito_horas` en
`core/sesiones.py`). Migraciones `permisos/0012_*` y `0013_*`.

Las decisiones y las alternativas descartadas están en el [ADR 014](./03-arquitectura/adr/014-horas-a-favor-del-explorador.md). En resumen:

- **Es una bolsa, no un ajuste a una deuda concreta.** El crédito sobrevive al pago y el
  remanente queda para deudas futuras. De ahí el consumo **FIFO** por `fecha_hecho`, que vive en
  el `ordering` del modelo (`:408`) y no en el servicio.
- **No es un tipo de transacción nuevo: es un medio de pago dentro del PDH existente.** Eso evita
  duplicar la vista, el servicio de pago y la lógica de reversión. `PDHCreateView` lo consume
  tras crear el pago y deshace el PDH entero si no se puede aplicar
  (`empleados/views/pdh.py:169-189`); `PDHDeleteView` lo devuelve a la bolsa (`:246`). La
  estructura es una **copia deliberada** de `DeudaPermisoMes` + `PagoDeudaPermisoMes`.
- **No toca la sanción.** Un mes vencido sigue sancionando aunque se salde con crédito: lo
  sancionable es haber dejado vencer el plazo, no el importe. `sancion_deuda_calculo.py` y
  `deuda_corporativa_service.py` quedaron **sin tocar**, y el centinela
  `permisos/tests/test_credito_horas.py:222` lo fija.

En el consolidado, `total_horas` se mantiene intacta —la leen la vista de morosos y las
plantillas— y el neto va aparte en `saldo_neto`, que **puede ser negativo** cuando la corporación
le debe a él (`turnos/services/consolidado_horas_service.py:286-303,330-333`).

Dos correcciones posteriores, del mismo día y del mismo error de base —validar en el sitio
equivocado—, están descritas en § 16.2: el desfase `min` / `step` del campo de horas y el
segundo mensaje de error contradictorio que producía `Model.clean()`.


### Levantar una auto-sanción por deuda condona la deuda del mes

Estado nuevo `'condonada'` en `DeudaCorporativa` (`solicitudes/models.py:719`) y
`DeudaPermisoMes` (`permisos/models.py:256`), y servicio
`DeudaCorporativaService.condonar_deudas_por_levantamiento`
(`solicitudes/services/deuda_corporativa_service.py:169`) disparado desde
`SancionEmpleado.levantar()` (`empleados/models.py:438`), dentro de su misma transacción.
Cierra un estado sin salida: la deuda de una sanción levantada no se podía pagar (mes
vencido), no la consumía esa sanción (`levantada_en IS NULL`) ni ninguna nueva (el mes ya
cuenta como juzgado). Se perdona la deuda, no el antecedente. Efectos colaterales
alineados: `_tiene_historia` (`permisos/deuda_permiso_service.py:44`) y el revert de PDH
(`permisos/pago_horas_service.py:387`) tratan `'condonada'` como hecho ocurrido, y el
consolidado de horas la muestra aparte, separada de lo cumplido
(`turnos/services/consolidado_horas_service.py:75,137`). Migraciones `permisos/0011_*` y
`solicitudes/0039_*`: solo `AlterField` de `choices`, sin `ALTER TABLE`. Razonado en el
[ADR 013](./03-arquitectura/adr/013-condonacion-al-levantar-la-sancion.md), que enmienda el
punto 3 del [ADR 011](./03-arquitectura/adr/011-sancion-independiente-del-pago.md).

En la misma tanda: **el criterio de "esta sanción es automática" pasa a ser el periodo en las
siete consultas que deciden algo** —dos `Q` de clase, `ES_AUTOMATICA` y `ES_MANUAL`
(`solicitudes/services/deuda_corporativa_service.py:58-69`)—, con lo que editar el motivo deja
de poder reabrir el ciclo de sanciones sobre un mes ya juzgado; el explorador **recibe el aviso
por la campana** (`notificar_condonacion`, `:282`, disparado desde `levantar()` dentro de la
misma transacción, `empleados/models.py:442-443`), con tipo de notificación propio
`'sancion_levantada'` (`solicitudes/models.py:22`, migración
`solicitudes/0040_alter_notificacion_tipo.py`); las horas que se comunican tras el hecho
salen de `horas_condonadas_por` (`:258`), que **lee lo escrito** en vez de recalcular la
estimación previa (`empleados/views/sanciones.py:149`); y el consolidado renombra
`total_saldado_sancion` → `total_extinguido_sancion`, `saldadas_sancion` →
`extinguidas_sancion` y `_saldadas_por_sancion` → `_extinguidas_por_sancion`
(`turnos/services/consolidado_horas_service.py:75,302-303`), alineando el código con lo que ya
decía la pantalla: **extinguido**, porque nadie pagó. Tests en
`solicitudes/tests/test_consumo_deuda_por_sancion.py:339,362,385` y
`turnos/tests/test_consolidado_extinguido_sancion.py`.
...
Secciones nuevas en [arquitectura-aws-rds-recomendada.md](./05-referencia/deployment/99-aws/arquitectura-aws-rds-recomendada.md):
...
Reparto de responsabilidades, sin cambio de comportamiento salvo donde se indica. Los cuatro
sitios que la auditoría señalaba como "cada corrección futura tendrá que entrar a ciegas".

| Commit | Qué cambió | Por qué |
|---|---|---|
| `7ebea0f` | **El orquestador de altas deja de hablar HTTP.** `SolicitudOrchestrator.procesar` devuelve `ResultadoSolicitud` (`solicitudes/services/resultado.py:32`) en vez de `JsonResponse`; la traducción vive en un único punto, `json_desde_resultado` (`solicitudes/views/resultado_http.py:12`), llamado desde `solicitudes/views/procesar_solicitud.py:30` | Crear una solicitud desde un test, un comando o una tarea obligaba a fabricar un POST y leer el cuerpo con `json.loads`. **El cuerpo JSON no cambia**: `como_payload()` reproduce lo que emitían `json_ok`/`json_error` (`resultado.py:81-93`) |
| `708e07a` | **Los ocho tramos de validación salen de `DobladaStrategy`** a `solicitudes/services/validators/doblada_flujo_validator.py`. `doblada_strategy.py` baja de 1.630 a 928 líneas | Los ocho métodos recibían `self` y ninguno lo usaba. No van a `doblada_validator.py` porque **devuelven** el mensaje en vez de lanzarlo, y `validar_pago_en_sabado` devuelve un `RequiereCambioTurnoPrevio` que un `ValidationError` aplanaría a texto (§ 7.2) |
| `5857373` | **El selector de fin de semana pertenece a dos estrategias, no a las seis.** `disponibilidad_companero` y `etiqueta_companero` salen de `SolicitudStrategy` al mixin `EstrategiaFinDeSemana` (`solicitudes/services/strategies/estrategia_fin_de_semana.py:47`) | La base tomaba partido por un subtipo: traía la regla del INTERCAMBIO por defecto, y D FDS —cuya regla es la CESIÓN— tenía que contradecir a su propia base. **Cambio de comportamiento**, ver la fila de `/solicitudes/dfds-companeros/` más abajo |
| `bb7c04d` | **Leer el formulario se separa de orquestar el alta** en DOBLADA PERMANENTE: `solicitudes/services/doblada_permanente_plan.py` parsea y valida la forma del alta multi-compañero; `_procesar_doblada_permanente_multi` baja de 236 a 150 líneas (`solicitud_orchestrator.py:366`) | — |
| `1dc0113` | **La decisión por día sale de la vista de Mis Turnos** a `turnos/services/mis_turnos_dia.py`; `MisTurnosPorMesView.get` baja de 345 a 176 líneas | 184 de esas 345 líneas eran el cuerpo de un solo bucle, en la pantalla que más gente mira. El contexto viaja en `ContextoMes` en vez de nueve parámetros sueltos |
| `fffed9e` | **La clasificación por empleado sale de `ReporteDiaService`** a `turnos/services/reporte_dia_empleado.py`; `reporte()` baja de 340 a 225 líneas | El servicio se queda con lo suyo: reunir en LOTE los datos del día para evitar las N+1. **El orden de las capas es el contrato** —replica el de `TurnoService.estado_dia`— y `turnos/tests/test_reporte_dia_paridad.py` detecta cualquier reordenamiento |
| `f2af451` | **Se retira la capa de repositorios.** Desaparecen `solicitudes/repositories/` y `empleados/repositories/`; sobrevive `SolicitudCambioQuerySet.con_relaciones()` (`solicitudes/models.py:149`) instalada como manager por defecto (`:256`) | De sus 14 métodos solo 2 tenían llamador. Se consolida en el camino nativo de Django, el mismo que ya seguían `EmpleadoQuerySet` (`empleados/models.py:41`) y `TurnoActivoManager` (`turnos/models.py:41`) |

### Cambios de comportamiento (no son refactor)

| Commit | Qué cambió | Efecto visible |
|---|---|---|
| `5857373` | **`GET /solicitudes/dfds-companeros/` devuelve 400 `tipo_sin_selector_finde`** si el tipo de solicitud no es de fin de semana (`solicitudes/views/api_fin_semana.py:65-69`) | Antes contestaba **200 con una lista calculada con la regla equivocada**: pedirle los compañeros de un sábado con el id de DOBLADA aplicaba la regla del intercambio y devolvía una lista verosímil y sin sentido, sin fallar y sin avisar. Ficha completa en § 6.2 |
| `7cdb192` | **El mensaje de rechazo por año operativo cambia de texto.** Ya no promete que la apertura de año desbloquee la fecha; dice que se podrá a partir del **1 de enero** (`core/utils/anio_operativo.py:63-67`) | La regla **no cambió**. El texto anterior mandaba al supervisor a completar el checklist esperando que le desbloqueara la fecha: la apertura *prepara* el año siguiente, no adelanta el momento de pisarlo. Quien seguía esa pista completaba los cinco ítems y se encontraba el mismo rechazo (`:57-62`) |
| `d5c1f7f` | **`PagoHorasService.deudas_pendientes` devuelve tres claves nuevas** por periodo: `fin_de_plazo`, `dias_restantes` y `urgente` (`permisos/pago_horas_service.py:184-186`) | Aviso **previo** al vencimiento, no solo el aviso de que ya cerró. `dias_restantes` vale 0 el último día hábil del plazo —hoy todavía se puede pagar— y es negativo si el mes ya venció; `urgente` marca la última semana (`:90-91`) |
| `311d281` | **`Turno.sala` y `TurnoArchivo.sala` admiten NULL** (migración `turnos/migrations/0016_alter_turno_sala_alter_turnoarchivo_sala.py`) | La interfaz muestra **"Por asignar"** cuando falta. Antes, un explorador sin competencia cargada hacía fallar la aprobación de la solicitud por un dato que no condiciona la operación. Detalle en § 5.3 |
| `48f7ae2` | **Nuevo `python manage.py verificar_crons`** | Comprueba desde fuera que los dos crons corren; marcador `CRON_NO_EJECUTADO` y salida distinta de cero. Ver § 7.3 y § 13.6 |
| `9627d7c`, `02e420e` | **Nuevas banderas `pytest --dias-en-el-futuro=N` y `--anio-planificado`** | Detectan los tests que caducan solos. Ver § 14.1 |
...
| Nota del PDH obligatoria | `empleados/forms.py:380,382-385` (`required = True` + `clean_comentario`), `empleados/views/pdh.py:91,155` | En la vista se valida **después** de fecha y explorador, a propósito, para no tapar esos mensajes |
...
| Front: `required` en las plantillas | `templates/solicitudes/solicitudes_pendientes_list.html:253`, `templates/empleados/pdh_create.html:79`, `templates/solicitudes/reprogramacion_registrar.html:49` | Comodidad, no defensa: la que cuenta es la del servidor |
...
**Tests que sujetan la regla.** `solicitudes/tests/test_cancelacion_lifo.py:167` y `:185` (pedir y
...
`solicitudes/tests/test_cancelacion_lifo.py:167` y `:185`, y
...
### El día de pago de la cobertura con 2 compañeros debe estar libre

Continuación directa del cambio anterior, sobre el mismo formulario. Regla completa en § 8.3.

El caso que lo destapó: cobertura de **día completo (2 compañeros)** con el día de pago puesto
en un día donde el solicitante ya trabajaba la AM. El backend lo rechazaba —bien— pero por la
jornada que chocaba, y el mensaje mandaba a hacer un **Cambio de Turno sencillo**. Ese consejo
no lleva a ninguna parte: las dos solicitudes comparten `fecha_pago`, así que ese día se pagan
las **dos** medias jornadas, y girar la jornada propia solo traslada el choque al otro
compañero. Si además el día fuera de mantenimiento, ahí no se puede pedir un CT siquiera.

| Cambio | Impacto |
|---|---|
| `SolicitudOrchestrator._pago_dos_debe_estar_libre`, llamado al inicio de `_procesar_cobertura_dos` | **Guard de pareja**: mira una condición que ninguna de las dos solicitudes ve por separado, así que no cabía en `_validar_semana_cobertura`. Corta antes de validar ninguna y da el motivo real más las dos salidas que existen (día libre, o solo AM / solo PM) |
| Recuadro rojo bloqueante `cob-aviso-dos-libre` + error al enviar, sobre `mi_jornada_pago` (dato que el endpoint de candidatos **ya** devolvía) | El usuario lo ve al elegir el día, no tras rellenar el formulario entero. Mismo criterio que el commit `e93ce3b` de arriba |
| En modo `DOS` se **suprime** el aviso «Ir a Cambio de Turno» (salida temprana de `verificarPagoCobertura`) | Sigue vivo en *Solo AM* / *Solo PM*, donde sí resuelve. Retirarlo del modo DOS es la mitad del arreglo: el consejo equivocado era el daño |
| El desplegable de día de pago **deshabilita los días de mantenimiento**, con el motivo a la vista | Ese día no opera nadie: no hay jornada del compañero que cubrir. Antes se podían elegir y el envío moría más adelante sin explicar por qué |
| `solicitudes/tests/test_cobertura_dos_companeros.py`: dos casos nuevos y uno reencuadrado | Uno de ellos fija que el mensaje **no** mande al Cambio de Turno. `test_si_falla_una_jornada_no_se_crea_ninguna` se reescribió para seguir probando la atomicidad por jornada: su escenario antiguo lo intercepta ahora el guard nuevo |
...
| `2a8e902` | **El pago en sábado exige que el receptor trabaje ese día.** Guard nuevo al principio de `_aplicar_pago_sabado`, antes de escribir (`solicitudes/services/doblada_pago_service.py:139-151`), con la excepción de que la mitad contraria ya esté cubierta por otra doblada aprobada (`:99-123`) | Cierra la vía por la que la aprobación por enlace de correo y la re-validación al aprobar creaban un turno en un día de descanso del receptor. Nueva `TestPagoSabadoReceptorDescansa` (`solicitudes/tests/test_matriz_dobladas.py:2635`); suite en verde, 936 tests |
| `2a8e902` | **La mitad contraria del sábado la puede cubrir cualquier deudor, no solo el mismo.** Las dos consultas que expresaban el criterio por separado (excepción del guard y rama `_contraria_tambien_cubierta`) se unifican en `DobladaPagoService._mitad_contraria_cubierta_por_otra_doblada` (`solicitudes/services/doblada_pago_service.py:99-123`), que **deja de filtrar por `explorador_solicitante`**. Se reformula el `ValidationError` del guard: describe el problema en vez de ordenar "elige otra fecha", porque también lo lee el aprobador (`:146-152`) | Corrige un bug **preexistente** de doble cobertura —el reparto recreaba al acreedor la mitad que otro deudor ya cubría— y otro del guard —rechazaba re-aplicaciones legítimas (`reconciliar_dobladas_aprobadas`, re-validación al aprobar, enlace de correo) cuando el acreedor ya se había quedado sin turnos—. Nuevos tests en `TestPagoSabadoReceptorDescansa`: `solicitudes/tests/test_matriz_dobladas.py:2759` y `:2795`; suite en verde, 938 tests |
...
| `2a8e902` | **`_otra_mitad_pagada` queda marcada como deliberadamente NO unificada** con `_mitad_contraria_cubierta_por_otra_doblada`, con el porqué en el código (`doblada_pago_service.py:195-199`) | Se parecen y preguntan lo contrario: deudor vs. acreedor. Sin la nota, el próximo refactor las funde y rompe una de las dos. Ver § 16.2 |
| `5e05312` | **Patrón 39 registrado** en `PROTECTION_PATTERNS.md:1651` ("la guardia vive donde está la ESCRITURA, no solo en la estrategia"), con cinco reglas y cuatro filas de registro de cambios | Cierra la fila 15 de § 18. Su regla 4 —`create()` dentro de `if not ...exists()` es señal de alarma— generaliza el bug del reparto de sábado. La limitación que registraba —"solo se auditó la rama de sábado"— quedó cerrada por la auditoría posterior de esta misma rama (§ 16.2) |
| `2a8e902` | **Auditadas las otras cinco ramas de `aplicar_doblada_pago`** (cierra la fila 16 de § 18). Resultado rama por rama: `_aplicar_pago_jcp_ambas` **limpia** en este eje (borra los turnos del acreedor y solo crea para el deudor, `doblada_pago_service.py:257-270`); `_aplicar_pago_jcp_media` (`:321-329`) y `_aplicar_pago_jornada_cedida` (`:509-519`) tienen la **misma forma** que el bug de sábado; `_aplicar_pago_cesion_parcial` (`:364`) y `_aplicar_pago_fallback` (`:540-554`) no fabrican turno pero validan con la jornada **base** en vez de con "¿trabaja?". **Sin cambio de comportamiento en esa tanda**: solo notas en el código | Auditoría. La conclusión de entonces —"ninguna tiene guardia propia y son inalcanzables gracias al LIFO, así que no se añaden guards"— **quedó revisada en la entrada siguiente**: el LIFO tapa la reconciliación, no los comandos de gestión |
...
| `4bddab7` | **Los días de descanso fijados de temporada solo se cambian desde CAMBIO DESCANSO** | Regla P3. Añade `DescansoSemanaService.es_dia_descanso_temporada` (`turnos/services/descanso_semana_service.py:44`) y el rechazo en `doblada_strategy.py:201-297` |
...
| 11 | Si las horas a favor deben **caducar** (p. ej. a los 6 meses) | Decisión de negocio, no de código | Hoy `CreditoHoras` **no caduca nunca**. El sitio natural para implementarlo sería un campo `vence_el` más un filtro en `creditos_disponibles` (`permisos/credito_horas_service.py:37`), pero activarlo requiere que el negocio lo decida |
| 12 | Si un crédito debería poder aplicarse **sin** un PDH asociado | `permisos/credito_horas_service.py:128-131` | Por diseño el crédito es un medio de pago **dentro** de un PDH y no puede superar lo que ese pago salda. No se ha planteado el caso de un explorador con crédito y sin ninguna deuda que saldar: hoy el saldo simplemente se acumula |
| 13 | Cobertura real del CRUD de crédito en navegador | Solo pruebas de Django (`Client`) | Los 23 casos de `permisos/tests/test_credito_horas.py` cubren servicio, formulario y vistas por HTTP, pero **no** el comportamiento del `<input type=number>` en un navegador real, que es justo donde apareció el fallo del `min`/`step` (§ 16.2) |
...
| 11 | Frecuencia real del cron de `procesar_email_outbox` en producción | [MANUAL_OUTBOX_CORREOS.md](./05-referencia/deployment/02-correo/MANUAL_OUTBOX_CORREOS.md) | No está en el código: es configuración de infraestructura |
| ~~23~~ | ~~Inconsistencia entre las dos vías de extinción de deuda por sanción: la condonación reconocía la auto-sanción por periodo y el consumo por el prefijo del motivo~~ | `solicitudes/services/deuda_corporativa_service.py:58-69` | **RESUELTO 2026-09-05** — el criterio por periodo se extendió a las **siete** consultas que deciden algo, con dos `Q` de clase (`ES_AUTOMATICA`, `ES_MANUAL`). El prefijo queda como etiqueta de lectura. Cierra además la vía de doble sanción sobre un mes ya juzgado. Test: `test_editar_el_motivo_no_reabre_el_ciclo_de_sanciones` (`solicitudes/tests/test_consumo_deuda_por_sancion.py:339`) |
